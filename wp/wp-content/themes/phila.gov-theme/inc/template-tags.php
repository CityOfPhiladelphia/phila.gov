...
function phila_gov_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'phila-gov' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'phila-gov' ),
...
function phila_gov_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( __( 'Leave a comment', 'phila-gov' ), __( '1 Comment', 'phila-gov' ), __( '% Comments', 'phila-gov' ) );
		echo '</span>';
	}
