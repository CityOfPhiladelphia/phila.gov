      ...
      foreach ($phone_label_types as $label) {
        if( !array_key_exists($label ,$meta)){
          $meta[$label] = '';
			...
			return sprintf(
				'(<input type="tel" name="%1$s[area]" id="%2$s_area" value="%3$s" size="3">)<input type="tel" name="%1$s[phone-co-code]" id="%2$s_phone-co-code" value="%4$s" size="3">-<input type="tel" name="%1$s[phone-subscriber-number]" id="%2$s_phone-subscriber-number" value="%5$s" size="4">',
				$field['field_name'],
				$field['id'],
				$meta['area'],
        $meta['phone-co-code'],
