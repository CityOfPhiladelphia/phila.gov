...
class Phila_Programs_Controller {
  ...
  */
   ...
   * @param WP_REST_Request $request Current request.
   */
  public function get_item_v2( $request ) {
    ...
    $response = $this->prepare_item_for_response_v2( $post, $request );
  ...
  public function prepare_item_for_response_v2( $post, $request ) {
    $post_data = array();

    $schema = $this->get_item_schema_v2( $request );
   ...
   * @param WP_REST_Request $request Current request.
   */
  public function get_item_schema_v2( $request ) {
    $schema = array(
      // This tells the spec of JSON Schema we are using which is draft 4.
      '$schema'              => 'http://json-schema.org/draft-04/schema#',
      // The title property marks the identity of the resource.
